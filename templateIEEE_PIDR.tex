...
L'algorithme repose sur le fait que tous nœud uniquement père ou uniquement fils ne fait pas partie d'un cycle. Par suppression successive de ces nœuds et arcs associes on peut déterminer qu'il y a un ou des cycles dans la ou les zones restantes.  \\
 ...
 \begin{algorithm}[H]
 \KwData{Graphe G contenant une liste de nœuds et une liste d'arc}
...
\end{itemize}
Ainsi que toutes les fonctions et procédures pour que c'est différents éléments ce mettent à jour automatiquement. \\
De plus comme il est écrit dans la partie \textcolor{red}{mettre le bon indice de partie} l'outil d'édition de réseaux bayésiens est une extension de l'outil DynGraph. C'est pour cela que nous n'avons pas choisi cette méthode car elle implique d'énormes modifications sur le code existant. \\

\subparagraph{Ajouter un graphe de réseau bayésien en parallèle du graphe de l'outil DynGraph \hfill }
\\ Cette seconde méthode est celle qui est implémenter. Les réseaux bayésiens et le calcul d'inférence on été rajoutés via l'ajout de l'outil Jayes (éditeur de réseaux bayésiens, Java, open source, sans interface graphique) dans l'outil DynGraph. \\  

\textcolor{blue}{Pas entièrement fini }\\

\paragraph{Découpage de l'édition de réseaux bayésiens \hfill}
\\
...
\subparagraph{Étape de mise en place des probabilités}
